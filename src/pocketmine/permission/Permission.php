...
class Permission{
	...
	public static $DEFAULT_PERMISSION = Permission::DEFAULT_OP;
	...
	public function __construct($name, $description = null, $defaultValue = null, array $children = []){
		...
		$this->defaultValue = $defaultValue !== null ? $defaultValue : Permission::$DEFAULT_PERMISSION;
	...
	public static function loadPermissions(array $data, $default = Permission::DEFAULT_OP){
		...
		foreach($data as $key => $entry){
			$result[] = Permission::loadPermission($key, $entry, $default, $result);
		}
	 ...
	 * @throws \Exception
	 */
	public static function loadPermission($name, array $data, $default = Permission::DEFAULT_OP, &$output = []){
		$desc = null;
		$children = [];
		...
		if(isset($data["children"])){
			if(is_array($data["children"])){
				foreach($data["children"] as $k => $v){
					if(is_array($v)){
						if(($perm = Permission::loadPermission($k, $v, $default, $output)) !== null){
							$output[] = $perm;
						}
					}
					$children[$k] = true;
