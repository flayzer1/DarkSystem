...
class ZippedResourcePack implements ResourcePack{
	...
	public function __construct(string $zipPath){
		...
		if(($manifestData = $archive->getFromName("pack_manifest.json")) === false){
			throw new \InvalidStateException("Could not load resource pack from $zipPath: pack_manifest.json not found");
		}
		...
		if(!self::verifyManifest($manifest)){
			throw new \InvalidStateException("Could not load resource pack from $zipPath: pack_manifest.json is invalid or incomplete");
		}
