<?php

#______           _    _____           _                  
#|  _  \         | |  /  ___|         | |                 
#| | | |__ _ _ __| | _\ `--. _   _ ___| |_ ___ _ __ ___   
#| | | / _` | '__| |/ /`--. \ | | / __| __/ _ \ '_ ` _ \  
#| |/ / (_| | |  |   </\__/ / |_| \__ \ ||  __/ | | | | | 
#|___/ \__,_|_|  |_|\_\____/ \__, |___/\__\___|_| |_| |_| 
#                             __/ |                       
#                            |___/

namespace pocketmine\entity;

interface EntityIds{

	const CHICKEN = 10;
	const COW = 11;
	const PIG = 12;
	const SHEEP = 13;
	const WOLF = 14;
	const DOG = 14;
	const VILLAGER = 15;
	const MOOSHROOM = 16;
	const SQUID = 17;
	const RABBIT = 18;
	const BUNNY = 18;
	const BAT = 19;
	const IRON_GOLEM = 20;
	const VILLAGER_GOLEM = 20;
	const SNOW_GOLEM = 21;
	const SNOWMAN = 21;
	const OCELOT = 22;
	const OZELOT = 22;
	const LEOPAR = 22;
	const CAT = 22;
	const HORSE = 23;
	const DONKEY = 24;
	const MULE = 25;
	const ILLUSIONER = 25; //???
	const SKELETON_HORSE = 26;
	const ZOMBIE_HORSE = 27;
	const POLAR_BEAR = 28;
	const BEAR = 28;
	const LLAMA = 29;
	const PARROT = 30;
	const BIRD = 30;

	const ZOMBIE = 32;
	const CREEPER = 33;
	const SKELETON = 34;
	const SPIDER = 35;
	const ZOMBIE_PIGMAN = 36;
	const PIG_ZOMBIE = 36;
	const SLIME = 37;
	const ENDERMAN = 38;
	const ENDERMEN = 38;
	const SILVERFISH = 39;
	const CAVE_SPIDER = 40;
	const GHAST = 41;
	const GHOST = 41;
	const MAGMA_CUBE = 42;
	const LAVA_SLIME = 42;
	const BLAZE = 43;
	const ZOMBIE_VILLAGER = 44;
	const WITCH = 45;
	const STRAY = 46;
	const HUSK = 47;
	const WITHER_SKELETON = 48;
	const GUARDIAN = 49;
	const ELDER_GUARDIAN = 50;
	const GUARDIAN_ELDER = 50;
	const BOSS_GUARDIAN = 50;
	const GUARDIAN_BOSS = 50;
	const NPC = 51;
	const WITHER = 52;
	const ENDER_DRAGON = 53;
	const DRAGON = 53;
	const SHULKER = 54;
	const ENDERMITE = 55;
	const LEARN_TO_CODE_MASCOT = 56;
	const VINDICATOR = 57;

	const ARMOR_STAND = 61;
	const TRIPOD_CAMERA = 62;
	const CAMERA = 62;
	const PLAYER = 63;
	const HUMAN = 63;
	const ITEM = 64;
	const DROPPED_ITEM = 64;
	const TNT = 65;
	const PRIMED_TNT = 65;
	const FALLING_BLOCK = 66;
	const FALLING_SAND = 66;
	const MOVING_BLOCK = 67;
	const XP_BOTTLE = 68;
	const THROWN_XP_BOTTLE = 68;
	const THROWN_EXP_BOTTLE = 68;
	const XP_ORB = 69;
	const EXP_ORB = 69;
	const EYE_OF_ENDER_SIGNAL = 70;
	const ENDER_CRYSTAL = 71;
	const FIREWORKS_ROCKET = 72;
	const FIREWORKS = 72;
	const FIREWORK = 72;

	const SHULKER_BULLET = 76;
	const FISHING_HOOK = 77;
	const CHALKBOARD = 78;
	const DRAGON_FIREBALL = 79;
	const ARROW = 80;
	const SNOWBALL = 81;
	const EGG = 82;
	const PAINTING = 83;
	const MINECART = 84;
	const LARGE_FIREBALL = 85;
	const SPLASH_POTION = 86;
	const THROWN_POTION = 86;
	const ENDER_PEARL = 87;
	const LEASH_KNOT = 88;
	const WITHER_SKULL = 89;
	const BLUE_WITHER_SKULL = 89;
	const BOAT = 90;
	const WITHER_SKULL_DANGEROUS = 91;
	const LIGHTNING_BOLT = 93;
	const LIGHTNING = 93;
	const SMALL_FIREBALL = 94;
	const AREA_EFFECT_CLOUD = 95;
	const HOPPER_MINECART = 96;
	const TNT_MINECART = 97;
	const CHEST_MINECART = 98;

	const COMMAND_BLOCK_MINECART = 100;
	const LINGERING_POTION = 101;
	const LLAMA_SPIT = 102;
	const EVOCATION_FANG = 103;
	const EVOCATION_FANGS = 103;
	const EVOCATION_ILLAGER = 104;
	const EVOKER = 104;
	const VEX = 105;
	
}
